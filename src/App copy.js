...
const renderField = (field, parent) => {
    ...
    switch (field.type) {
        ...
        case 'catalog-select-conditional':
            ...
            const pk = 'parent-' + field.key

            const handleSelection = (field, value) => {
                const selectedPathLocales = value.path_locales;
            
                // Actualiza la selección activa
                setActiveSelection(selectedPathLocales);

                // if( parent != null ){
                  // console.log( field, parent);
                  setRenderedChildren((prev) => {
                      const fieldRendered = prev[pk] || [];
                      if (!fieldRendered.includes(selectedPathLocales)) {
                          return {
                              ...prev,
                              [field.key]: 'type::' + (selectedOption?.key ?? ''),
                          };
                      }
                      return prev;
                  });
                      ...
                      if (selectedPathLocales === conditional.caseOf) {
                          return conditional.nestChildren.map((nestedField) => {
                            // console.log(renderedChildren[field?.key ?? ''] === nestedField.key, field?.key ?? '', nestedField.key);
                              ...
                              // Si es una selección nueva o activa, renderiza los hijos
                              if (!isAlreadyRendered || activeSelection === selectedPathLocales) {
                                  return renderField(nestedField, field);
                              }
